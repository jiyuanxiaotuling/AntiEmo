...
export default {
  data() {
    return {
      ...
      oldConv: undefined,
      convTitletmp: "",
      user_id: "user123", // 示例用户ID，可动态生成或从登录信息获取
  ...
  methods: {
    ...
    stopChat() {
      var that = this;
      this.axios.put(`/stop/chat/${this.session_id}`, {})
        .then((result) => {
    ...
    suitable(idx, conv, suit) {
      ...
      this.axios.put(`/ai/suitable/${this.session_id}`, cdate)
        .then((result) => {
          console.log(result);
    ...
    async chatRepeat() {
      ...
      try {
        ...
        console.error('Error repeating chat:', error.response ? error.response.data : error);
    ...
    .then((response) => {
      ...
      if (first) {
        that.session_id = that.session_id || `session_${Date.now()}`; // 使用临时值或后端返回的 session_id
        var newConv = {
          "id": that.session_id,
          "title": "New chat"
        };
  ...
  async generateConvTitle(conv) {
    var that = this;
    try {
      // 使用 axios 发送请求，假设后端接口为 /chat/title/{session_id}
      const response = await axios.get(`/chat/title/${this.session_id}`);
      
      // 假设后端返回格式为 { "title": "生成的标题" }
      conv.title = response.data.title || "默认标题"; // 设置标题，添加默认值以防后端返回空
      
      // 更新会话并保存
      that.selectConversation(conv, false);
      that.saveConversations();
    } catch (error) {
      console.error('Error generating title:', error.response ? error.response.data : error);
    ...
    // 假设后端提供接口获取历史对话
    this.axios.get(`/history/${this.user_id}/${this.session_id}`)
      .then((result) => {
        console.log(result);
        var resp = result.data;
